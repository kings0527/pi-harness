...
test("ADR-0011 through ADR-0030 遵守十行格式", () => {
  for (const path of [
    ...
    "docs/decisions/0029-board-collaboration-state-machine.md",
    "docs/decisions/0030-explicit-context-budget-ladder.md",
  ]) {
...
test("context-feed session_start 不递归清点 cwd", () => {
  ...
  assert.match(contextSource, /buildCatalogReference/);
  assert.match(contextSource, /buildBoardDelivery\(state\.participatingTopics, coverage\)/);
  assert.match(contextSource, /pi-harness-board-participation/);
  assert.match(contextSource, /catalog-only/);
  assert.match(contextSource, /PI_BOARD_CATALOG_MAX_TOPICS/);
